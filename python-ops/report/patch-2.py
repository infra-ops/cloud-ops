...
def cmds(ip,usr,pwd):
	...
	ip2="sshpass -p "+pwd+" ssh -o StrictHostKeyChecking=no -t -q " + usr+"@"+ip+" \'echo \'"+  pwd + "\'| sudo -S yum check-update\'"
#              sshpass -p 'deploy@123' ssh -o StrictHostKeyChecking=no deploy@ip-10-0-5-94.ec2.internal 'echo 'deploy@123' | sudo -S yum check-update'
	#     sshpass -p deploy@123 ssh -o StrictHostKeyChecking=no deploy@ip-10-0-5-94.ec2.internal echo "${deploy@123}" | sudo -S yum check-updatei

...
parser.add_argument('-u',action='store',dest='uuid',default=None,help='OPTIONAL: User unique id for lm-service-couchbase')
...
def deleteService(mode,confFile,lmsUuid,region,clusterName):
    ...
    for modules in confParser['containers']:
        for svc in modules['serviceDelete']:
            ...
            if lmsUuid:
               svcServiceName = 'lm-service-'+lmsUuid
            try:
                if mode:
...
def registerNUpdateTask(mode,confFile,lmsUuid,region,count):
    ...
    for modules in confParser['containers']:
        ...
        try:
           ...
           networkMode = 'bridge'
        if lmsUuid:
           # To be used for LMS only
           family = 'LM-'+lmsUuid
           uuidEnv = {'name': 'UUID','value':lmsUuid}
           # We require an env variable corresponding to service name for registrator
           # That is how we would see the service in consul registry
           snl='lm-'+lmsUuid
           serviceNameLms = {'name': 'SERVICE_NAME', 'value': snl }
           # Append dict to list here
           for defs in containerDefinitions:
               for keys,value in defs.iteritems():
                   if keys == 'environment':
                      value.append(uuidEnv)
                      value.append(serviceNameLms)
        # verbose
        #if mode:
       ...
       for modules in confParser['containers']:
           for svc in modules['serviceCreate']:
               ...
               if lmsUuid:
                  svcServiceName = 'lm-service-'+lmsUuid
                  svcTaskDefinition = 'LM-'+lmsUuid
                  svcClientToken = lmsUuid 
                  # Count for lmuser service is 1 for now
                  svcDesiredCount = 1
               try:
...
if __name__ == '__main__':
   ...
   if args.create:
      ...
      if lmsUuid:
         serviceConfFile = os.path.dirname(os.path.abspath(__file__))+"/config/"+args.env+"/lm-service.yaml"
         if mode:

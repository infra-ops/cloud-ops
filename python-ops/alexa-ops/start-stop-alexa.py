...
#	command = event["command"]
	...
	if command.lower() == "start":
		...
		speak="Start command finished at " + datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
		response = {"version": "1.0","response": {"outputSpeech": {"ssml": "<speak>"+speak + "</speak>","type": "SSML"},"card": {"content": speak ,"title": "Controlling Instances","type": "Simple"},"speechletResponse": {"outputSpeech": {"ssml": "<speak>"+speak+ "</speak>"},"shouldEndSession": "true"}},"sessionAttributes": {}}
		return response
		...
		speak = "Stop command finished at " + datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") + "\n"
		response = {"version": "1.0","response": {"outputSpeech": {"ssml": "<speak>"+speak + "</speak>","type": "SSML"},"card": {"content": speak ,"title": "Controlling Instances","type": "Simple"},"speechletResponse": {"outputSpeech": {"ssml": "<speak>"+speak+ "</speak>"},"shouldEndSession": "true"}},"sessionAttributes": {}}
		return response

...
def describe_ips():
	...
	for region in response['Regions']:
		...
		addresses_response = client.describe_addresses()
		if len(addresses_response['Addresses']) > 0:
			log_content.append('{}'.format(region['RegionName']))
		for address in addresses_response['Addresses']:
			log_content.append("{} {}".format(address['AllocationId'], address.get('NetworkInterfaceId', '')))
...
if __name__ == '__main__':
	...
	if eip is None and json_file is None:
		describe_ips()
	else:

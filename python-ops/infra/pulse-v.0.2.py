...
def tokenv():
 with requests.session() as q:
  if os.path.exists("%s/temp.txt"%cwd) and os.path.getsize("%s/temp.txt"%cwd) > 0:
	...
	foo= open("temp.txt","w+")
        da={"username":"sudipta@searchstax.co","password":"redmi@1234"}
        fl = q.post("https://app.searchstax.co/api/rest/v1/obtain-auth-token/",data=da)
        tok = json.loads(fl.text)
        apk= "Token %s"%tok["token"]
        log = q.get("https://app.searchstax.co/api/rest/v2/account/TDQ53Q02KT/deployment/",headers={"Authorization":apk})
...
class SearchTestCase(base_test.WebDriverTestCase):
    def login_for_chart(self):
        ...
        username_element.send_keys("xxxxxxxx")
	...
	dataT={"hostname[]":"%s-1"%host,"metricname[]":"os.SystemLoadAverage","startDate":"%s"%dates['start_time'],"endDate":"%s"%dates['end_time'],"daterangepicker":"Last 1 hour"}
    ...
    def testMemory(self):
	dataT={"hostname[]":"220736-1","precision":"GB","metricname[]":"os.TotalPhysicalMemorySize","metricname[]":"os.TotalPhysicalMemorySize-os.FreePhysicalMemorySize","startDate":"%s"%dates['start_time'],"endDate":"%s"%dates['end_time'],"daterangepicker":"Last 1 hour"}
        url = "https://app.searchstax.co/admin/deployment/pulse/deployment/ss220736/graph/%s"%str(urllib.urlencode(dataT))
        chart_has_data = self.checkChartActivity(url,dataT)
        self.assertTrue(chart_has_data)
    def testThreadCount(self):
        dates = self.makeDates()
        url = "https://app.searchstax.co/admin/deployment/pulse/deployment/ss276718/graph/hostname%5B%5D=ss276718-1&precision=int&collection=&metricname%5B%5D=jvm.ThreadCount&startDate=" + dates['start_time'] + "&endDate=" + dates['end_time'] + "&daterangepicker=Last+1+hour"
        chart_has_data = self.checkChartActivity(url)
        self.assertTrue(chart_has_data)
    def testSwap(self):
        dates = self.makeDates()
        url = "https://app.searchstax.co/admin/deployment/pulse/deployment/ss276718/graph/hostname%5B%5D=ss276718-1&precision=GB&collection=&metricname%5B%5D=os.TotalSwapSpaceSize&metricname%5B%5D=os.TotalSwapSpaceSize-os.FreeSwapSpaceSize&startDate=" + dates['start_time'] + "&endDate=" + dates['end_time'] + "&daterangepicker=Last+1+hour"
        chart_has_data = self.checkChartActivity(url)
        self.assertTrue(chart_has_data)
    def testHeapMemory(self):
        dates = self.makeDates()
        url = "https://app.searchstax.co/admin/deployment/pulse/deployment/ss276718/graph/hostname%5B%5D=ss276718-1&precision=GB&collection=&metricname%5B%5D=jvm.heapMemoryUsage.coitted&metricname%5B%5D=jvm.heapMemoryUsage.used&startDate=" + dates['start_time'] + "&endDate=" + dates['end_time'] + "&daterangepicker=Last+1+hour"
        chart_has_data = self.checkChartActivity(url)
        self.assertTrue(chart_has_data)
    def testNonHeapMemory(self):
        dates = self.makeDates()
        url = "https://app.searchstax.co/admin/deployment/pulse/deployment/ss276718/graph/hostname%5B%5D=ss276718-1&precision=GB&collection=&metricname%5B%5D=jvm.nonHeapMemoryUsage.coitted&metricname%5B%5D=jvm.nonHeapMemoryUsage.used&startDate=" + dates['start_time'] + "&endDate=" + dates['end_time'] + "&daterangepicker=Last+1+hour"
        chart_has_data = self.checkChartActivity(url)
        self.assertTrue(chart_has_data)
    """
